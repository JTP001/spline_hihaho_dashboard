...
function Users() {
    ...
    const [dataView, setDataView] = useState("Users");
    ...
    useEffect(() => {
        if (user.is_superuser) {
            ...
            axiosInstance.get("user/logs/")
                .then(res => {
                    const logs = res.data.results.map(log => ({
                        ...log,
            ...
            {user && user.is_superuser ? (
                <div className="d-flex flex-column">
                    ...
                    <div className="my-2 d-flex flex-row flex-wrap justify-content-around">
                        <button className="btn bg-info-subtle shadow-sm" onClick={() => setDataView("Users")}>Users List</button>
                        <button className="btn bg-info-subtle shadow-sm" onClick={() => setDataView("Logs")}>User Logs</button>
                    </div>
                    {dataView === "Users" &&
                        <>
                            ...
                            <Box key={listUser.id} className="my-2 mx-auto w-75">
                                <Paper className="p-2 bg-info-subtle">
                                    <div className="d-flex flex-row justify-content-around align-items-center">
                                        ...
                                        <div className="d-flex flex-column flex-grow-1" style={{"maxWidth":"30%", whiteSpace: "normal", wordBreak: "break-word"}}>
                                            <p>Username:</p>
                                        ...
                                        <div className="d-flex flex-column">
                                            <button className="btn" style={{"backgroundColor":"tomato", color:"white"}} onClick={() => handleDeleteUser(listUser.id)}>
                                                Delete
                                            </button>
                        ...
                        {userLogs.slice(0, 10).map(log => (
                            <Box key={log.id} className="my-2 mx-auto w-75">
                                <Paper className="p-2 bg-info-subtle">
                                    <div className="d-flex flex-row justify-content-around align-items-center">
                                        <div className="d-flex flex-column flex-grow-1" style={{"maxWidth":"30%", whiteSpace: "normal", wordBreak: "break-word"}}>
                                            <p>Username:</p>
                                            <h6>{log.user.username}</h6>
                                        </div>
                                        <div className="d-flex flex-column flex-grow-1" style={{"maxWidth":"30%", whiteSpace: "normal", wordBreak: "break-word"}}>
                                            <p>Log message:</p>
                                            <h6>{log.message || "No message"}</h6>
                                        </div>
                                        <div className="d-flex flex-column flex-grow-1" style={{"maxWidth":"30%", whiteSpace: "normal", wordBreak: "break-word"}}>
                                            <p>Timestamp:</p>
                                            <h6>{log.timestamp.toString()}</h6>
                                        </div>
                                    </div>
                                </Paper>
                            </Box>
                        ))}
                        {userLogs.length >= 10 && <h6 className="mx-auto my-1">*Showing only the 10 most recent logins</h6>}
                        </>
                    }
                </div>
            ) : (
